...
function listProcurementsAccounts() {
	...
	axios
		.get(`${apiPath}getApplications/${id}`, {
			headers: {
				Authorization: token,
			},
		})
		.then(function(response) {
            ...
            data[0].fields.map((itm, indx) => {
                head += `<td>${itm.fieldName}</td>`;
             })

			if (data.length !== 0) {
				data.map((item, indx) => {
                    // <!-- <th style="width:10%;"><input type="checkbox" name="" id=""></th> -->
                    // <th>Code</th>
                    // <th>Date</th>
                    // <th>Vendor</th>
                    // <!-- <th>Batch Total</th> -->
                    ...
                    item.fields.map((itm, indx) => {
                       res += `<td>${itm.response}</td>`;
                    })
